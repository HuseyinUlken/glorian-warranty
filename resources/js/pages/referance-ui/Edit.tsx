...
interface EditCashBoxProps extends PageProps {
    cashBox: CashBox;
    branch: {
        id: number;
        name: string;
...
export default function EditCashBox({ cashBox, branch, cashBoxTypes, currencyTypes }: EditCashBoxProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('cash-boxes.update', cashBox.id), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Kasa Düzenle: ${cashBox.name}`} />
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <Heading
                    title={`Kasa Düzenle: ${cashBox.name}`}
                    description={`${branch.name} şubesine ait kasayı düzenlemek için bu formu kullanabilirsiniz.`}
                    buttons={
